...
const BlogTableltem = ({blog, fetchBlogs, index}) => {
 ...
 const deleteBlog = async () =>{
  const confirm = window.confirm('Bu blogu silmek istediğinizden emin misiniz?')
  if (!confirm) return; 
  try {
  ...
  return (
    <tr className='border-y border-gray-300'>
     ...
     <td className='px-2 py-4 flex text-xs gap-3'>
      <button onClick={togglePublish} className='border px-2 py-0.5 mt-1 rounded cursor-pointer'>{blog.isPublished ? 'Yayından kaldır' : 'Yayınla'}</button>
